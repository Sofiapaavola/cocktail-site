...
const MockCocktailList = (AddFavourite) => { 
    return (
    <Router>
        <CocktailList 
        results={cocktails} 
        favouriteComponent={favourite}
        handleFavouritesClick={mockedHandleClick}
        />
    </Router>
    )
}

it('renders correct title for "View Instructions" button', async () => {
  render(
    <MockCocktailList />
    );
  const title = screen.getByText(/View Instructions/i);
